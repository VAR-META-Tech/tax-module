...
func MapInvoiceToViettel(invoice *domain.Invoice, cfg config.ThirdPartyConfig, sellerCfg config.SellerConfig) *ViettelInvoiceRequest {
	...
	// Determine payment method — use invoice value if set, fallback to "TM/CK"
	...
	req := &ViettelInvoiceRequest{
		...
		BuyerInfo: BuyerInfo{
			...
			BuyerAddressLine: invoice.BuyerAddress,
		...
		SummarizeInfo: SummarizeInfo{
			SumOfTotalLineAmountWithoutTax: float64Ptr(invoice.TotalAmountWithoutTax),
			TotalAmountWithoutTax:          float64Ptr(invoice.TotalAmountWithoutTax),
			TotalTaxAmount:                 float64Ptr(invoice.TotalTaxAmount),
			TotalAmountWithTax:             float64Ptr(invoice.TotalAmountWithTax),
		},
	}
...
func mapItems(items []*domain.InvoiceItem) []ItemInfo {
	...
	for i, item := range items {
		...
		info := ItemInfo{
			...
			ItemTotalAmountWithoutTax: float64Ptr(item.ItemTotalAmountWithoutTax),
			TaxPercentage:             float64Ptr(item.TaxPercentage),
			TaxAmount:                 float64Ptr(item.TaxAmount),
			ItemTotalAmountWithTax:    float64Ptr(item.ItemTotalAmountWithTax),
		...
		if item.ItemTotalAmountAfterDiscount != nil {
			info.ItemTotalAmountAfterDiscount = item.ItemTotalAmountAfterDiscount
		}
		if item.ItemDiscount != nil {
			info.ItemDiscount = item.ItemDiscount
		}
		if item.Discount != 0 {
...
func buildTaxBreakdowns(items []*domain.InvoiceItem) []TaxBreakdown {
	...
	for _, item := range items {
		tb, ok := groups[item.TaxPercentage]
		if !ok {
			tb = &TaxBreakdown{
				TaxPercentage: float64Ptr(item.TaxPercentage),
			...
			groups[item.TaxPercentage] = tb
		}
		*tb.TaxableAmount += item.ItemTotalAmountWithoutTax
